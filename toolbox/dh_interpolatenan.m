function dataout = dh_interpolatenan(cfg, datain)
...
cfg.verbose     = ft_getopt(cfg, 'verbose', 0);          % default is off
...
for i=1:ntrl
  ...
  if any(replace(:)) % Check whether any values should be interpolated
    ...
    for j=1:size(idx_start_c, 1)
      ...
      if cfg.useSIGNI == 1
          if cfg.verbose
              disp('initiaizing SIGNI method, overriding FT default methods');
          end
          lengthNaNChunk = length(idx_start_c(j):idx_end_c(j));
          prewindowNew = lengthNaNChunk;
          postwindowNew = lengthNaNChunk;
          presampleIdx = idx_start_c(j)-prewindowNew:idx_start_c(j)-1;
          postsampleIdx = idx_end_c(j)+1:idx_end_c(j)+postwindowNew;
          %disp(idx_start_r(j))
          presampleData = fliplr(datain.trial{i}(idx_start_r(j), presampleIdx)); 
          if any(isnan(presampleData))
              %disp(j)
              %presampleData = fillmissing(presampleData,'linear',2,'EndValues','nearest'); 
              presampleData = inpaint_nans(presampleData);
          end
          postsampleData = fliplr(datain.trial{i}(idx_start_r(j), postsampleIdx)); 
          if any(isnan(postsampleData))
              %postsampleData = fillmissing(postsampleData,'linear',2,'EndValues','nearest');
              postsampleData = inpaint_nans(postsampleData);
          end
          
          taperReverse = fliplr([0:(1/lengthNaNChunk):1]);
          taperForward = [0:(1/lengthNaNChunk):1];
          presampleDataTaperReversed = presampleData .* taperReverse(1:lengthNaNChunk);
          postsampleDataTaperReversed = postsampleData .* taperForward(1:lengthNaNChunk);
          combinedSignal = presampleDataTaperReversed + postsampleDataTaperReversed;
          
          if any(isnan(combinedSignal))
            ft_error('There is still NaN in the data, check code');
          end
         
          % replace nan chunk with combined signal
          fill_window = idx_start_c(j):idx_end_c(j); % Indices of time-points that will be interpolated
          fill = combinedSignal;
          dataout.trial{i}(idx_start_r(j), fill_window)=fill;
          
%           
%           newPlot = dataout.trial{i}(idx_start_r(j), idx_start_c(j)-100:idx_start_c(j)+100);
%           
%          
%           plot(newPlot); hold on;
%           plot(datain.trial{i}(idx_start_r(j), idx_start_c(j)-100:idx_start_c(j)+100)); hold on;
%           
